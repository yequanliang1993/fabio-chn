...
p0 = 130;  % 130 products in FABIO
r = 31;  % 31 provinces in China
ts = 29;  % time period 1990-2018

cd('C:\Users\YeQ\Documents\MATLAB\Chinese FABIO\Data\FAOSTAT')
...
%% Commodity balance sheets (CBS) from FAOSTAT
...
element = {'Production';'Import Quantity';'Stock Variation';'Export Quantity';...
    ...
    'Food supply quantity (tonnes)';'Other uses'};
...
data_avi_fao(crop_a,:) = zeros(length(crop_a),length(element));
...
data = importdata(filename);            % only contains the production of live animals
...
save('data_avi_fao','data_avi_fao')
%% statistics data for production
cd('C:\Users\YeQ\Documents\MATLAB\Chinese FABIO\Data\China_Data\Crop production in provinces')
...
cd('C:\Users\YeQ\Documents\MATLAB\Chinese FABIO\Data\China_Data\Animal Slaughtered and products')
...
cd('C:\Users\YeQ\Documents\MATLAB\Chinese FABIO\Data\China_Data\Animal in-stock')
filename = 'Camels.xls';
data = importdata(filename);
...
data_offal = importdata(filename);
filename = 'Fats_of_animals.xlsx';
data_fat = importdata(filename);
...
data_hid_skin = importdata(filename);
filename = 'Freshmeat_animal.xlsx';
data_freshm = importdata(filename);
...
cd('C:\Users\YeQ\Documents\MATLAB\Chinese FABIO\Data\China_Data\Other products')
...
%% import and export of statistics data
cd('C:\Users\YeQ\Documents\MATLAB\Chinese FABIO\Data\China_Data\Import and Export of products')
fname = 'Import.xlsx';
...
cd('C:\Users\YeQ\Documents\MATLAB\Chinese FABIO\Data\Water footprint')
w_req = xlsread('Water for live animals.xlsx','A3:H12');
w_req = w_req(:, [1 5 8]);
% drinking and service water requirement, distinguishing industrial and grazing, 
% unit: litre per animal pre day,
...
wat_live_ani = zeros(ts,size(w_req,1),r);
for t = 1:ts
    for j = 1:p0  % for each crop as feeds
        ...
        if any(w_req(:,1) == j)
            for i = 1:r
                temp_ani_1 = prod_stat(t,j,i); % animal slaughted
                temp_ani_2 = ani_stock(t,j,i); % animal in-stock
                temp_ani_1(isnan(temp_ani_1)) = 0;
                temp_ani_2(isnan(temp_ani_2)) = 0;
                
                if any(prov_graz == i)
                    wat_live_ani(t,j-96,i) = (temp_ani_1*0.5+temp_ani_2)*w_req(j-96,3)*365/10^5;
                else
                    wat_live_ani(t,j-96,i) = (temp_ani_1*0.5+temp_ani_2)*w_req(j-96,2)*365/10^5;
                end
                
                clear temp_ani_1 temp_ani_2
            end
        end
...
%% product for seeds
cd('C:\Users\YeQ\Documents\MATLAB\Chinese FABIO\Data\China_Data\Crop sown areas in provinces')
...
data = importdata('Sown areas of other products.xlsx');
for j_c = 1:size(prod_c,1)
...
%% population
cd('C:\Users\YeQ\Documents\MATLAB\Chinese FABIO\Data\China_Data\Population')
data = importdata('Population total 1986-2018.xls');
...
%% trade-cost between capital cities
cd('C:\Users\YeQ\Documents\MATLAB\Chinese FABIO\Data\China_Data\Inter-provincial trade cost')
...
%%
cd('C:\Users\YeQ\Documents\MATLAB\Chinese FABIO')
save('FABIO_CHN','commod_bal','cost','data_avi_fao','element','exp_stat',...
    'feed_ani_31','imp_stat','meta','pop','prod_c','prod_stat','proc_type','proc_est','seed_est',...
    'sown_area','wat_live_ani','sown_area_a','sup_tab','sup_tab_chn','tcf_fao','use_tab')
